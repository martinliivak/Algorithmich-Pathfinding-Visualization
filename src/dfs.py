...
class DFS:
    """
    Depth-first search solver for maze graph
    """

    def __init__(self, graph, start, goal):
        """
        Initializing all of the parameters necessary to solve the maze using DFS """
        self.graph = graph
        self.start = start
        self.goal = goal
    ...
    def __next__(self):
        ...
        while self.stack:
            ...
            for next_item in self._get_neighbours(vertex):
                if next_item not in self.came_from and next_item not in self.visited:
                    self.came_from[next_item] = vertex
                    self.stack.append((next_item, val + 1))
